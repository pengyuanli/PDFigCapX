...
# Obtain all html information
    ...
    if os.path.isfile(html_info_json):
        ...
        browser = webdriver.Chrome(
            '/usa/pengyuan/Documents/RESEARCH/PDFigCapX/chromedriver/chromedriver')
        for page_id in range(page_no):
            ...
            for element in text_elements:
                text = element.text
                if len(text) > 0:
                    text_boxes.append([[element.location['x'], element.location['y'],
                                      element.size['width'], element.size['height']], text])
            html_info.append(
...
# obtain text layout
    ...
    if left_bar > 0 and left_bar < 20 * info['row_height']:
        ...
        info['right_bbox'] = [info['page_width'] - info['row_height'],
                              0, info['row_height'], info['page_height']]
        info['top_bbox'] = [0, 0, info['page_width'], info['row_height']]
        info['down_bbox'] = [0, info['page_height'] -
                             info['row_height'], info['page_width'], info['row_height']]
