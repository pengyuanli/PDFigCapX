    ...
    for page in sorted(os.listdir(html_file_path)):
        if page.endswith('.png') and page.startswith('page'):
            ...
            ret, thresh = cv2.threshold(
                imgray, 240, 255, cv2.THRESH_BINARY_INV)
            ...
            for cnt in contours:
                ...
                thresh_for_figure = info['row_height'] * \
                    png_ratio * 1.5  # / 2  modified on 0318
            ...
            if len(potential_bbox) > 1:
                ...
                no_of_small = len(
                    [1 for obj_height in obj_heights if obj_height < 13 and obj_height > 4])
            ...
            if info['mess_up'] == False:  # Need to set carefully
                ...
                while count < len(potential_bbox):
                    flag = 0
...
def fig_cap_matching(cap_box, fig_box, info, table_box, text_box):
    ...
    for i in range(info['page_no']):
        ...
        for table_cap in table_caps:  # To remove the table
            table_cap_box = [table_cap[0], table_cap[1] + table_cap[3],
                             table_cap[2], 4 * info['row_height']]  # Remove the table below
            p_figure_id = 0
        ...
        if len(p_figures) > 0:
            if len(p_figures) == 1 and len(p_captions) == 1:
                if p_figures[0][2] > fig_size_thresh and p_figures[0][3] > fig_size_thresh:  # size
                    if bbox_distance(p_figures[0], p_captions[0]) < 50:  # distance
                        ...
                        captions[page] = [
                            [p_captions[0], [1, 1, info['page_width'] - 2, info['page_height'] - 2]]]
            ...
            if len(p_captions) == 0:  # No caption situation
                ...
                page_width = info['page_width'] - \
                    info['left_bbox'][2] - info['right_bbox'][2]
                page_height = info['page_height'] - \
                    info['top_bbox'][3] - info['down_bbox'][3]
                if float(sum_area) / (page_width * page_height) > 0.2 and i > 1:
                    captions[page] = [[info['down_bbox'], [
    ...
    if len(cap_box) == 1:
        ...
        if column_no == 1:
            ...
            for cap_item in cap_sorted:
                region = [1, columns_point[0], info['page_width'] -
                          2, cap_item[1] - columns_point[0]]
                cap_regions.append([cap_item, region])
                columns_point[0] = cap_item[1] + cap_item[3]
            cap_regions.append([cap_item, [
                               1, columns_point[0], info['page_width'] - 2, info['page_height'] - columns_point[0]]])
        else:
            ...
            for cap_item in cap_sorted:
                ...
                if cap_item[2] > info['row_width'] + 50 or (cap_item[0] < info['page_width'] / 2 and
                    ...
                    region = [1, max(columns_point), info['page_width'] -
                              2, cap_item[1] - max(columns_point)]
                    ...
                    if no_cross_fig == 1:
                        if cap_x < columns[0] + 100:
                            region = [cap_x, columns_point[0],
                                      info['row_width'], cap_y - columns_point[0]]
                            columns_point[0] = cap_y + cap_item[3]
                        elif cap_x < columns[0] + info['row_width'] - 100:
                            region = [1, columns_point[0], columns[0] +
                                      info['row_width'], cap_y - columns_point[0]]
                            columns_point[0] = cap_y + cap_item[3]
                        else:
                            region = [min(cap_x, columns[0] + info['row_width'] + 50), columns_point[1], info['page_width'] - min(
                                cap_x, columns[0] + info['row_width'] + 50), cap_y - columns_point[1]]
                            columns_point[1] = cap_y + cap_item[3]

                    cap_regions.append([cap_item, region])
            # Added to cover all area, for image below captions
            if no_cross_fig == 0:
                region = [1, max(columns_point), info['page_width'] - 2,
                          info['page_height'] - max(columns_point)]
                cap_regions.append([cap_item, region])
            else:
                cap_regions.append([cap_item, [
                                   0, columns_point[0], info['page_width'] / 2, info['page_height'] - columns_point[0] - 1]])
    ...
    for i in range(len(cap_regions)):
        if len(labeled_figures[str(i)]) > 0:
            if len(labeled_figures[str(i)]) < 2:
                # Fig Thresh
                if labeled_figures[str(i)][0][2] > 20 and labeled_figures[str(i)][0][2] > 20:
                    fig_merged.append(
                ...
                for each_figure in labeled_figures[str(i)]:
                    ...
                    sum_figure_area = each_figure[2] * \
                        each_figure[3] + sum_figure_area
                ...
                if new_fig[2] > 20 and new_fig[3] > 20:  # Fig Threshold
                    # Check overlap ratio
                    overlap_fig = float(sum_figure_area) / \
                        (new_fig[2] * new_fig[3])
                    if overlap_fig > 0.1:
...
def evaluation(prefigures, cap_regions, html_file_path, info, html_boxes):
    ...
    for page in fig_cap_pair:
        ...
        for each_figcap in fig_cap_pair[page]:
            ...
            caption_flag = overlap_ratio_based(
                info['down_bbox'], each_figcap[1])
            if caption_flag > 0.8:
                ...
                x1 = max(each_figcap[0][0] + each_figcap[0]
                         [2], each_figcap[1][0] + each_figcap[1][2])
                y1 = max(each_figcap[0][1] + each_figcap[0]
                         [3], each_figcap[1][1])
        ...
        for each_figure in figures[page]:
            if len(each_figure[1]) > 0:
                ...
                if each_figure[0][2] > 1.5 * info['row_width'] and each_figure[1][0][1] > each_figure[0][1] + each_figure[0][3] \
                    ...
                    each_figure[1][0][2] = 2 * \
                        each_figure[1][0][2] + 2 * info['row_height']
