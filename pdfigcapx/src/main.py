...
def get_pages(xpdf_path: Path):
    ...
    page_names = [x for x in listdir(xpdf_path) if x.endswith(".html") and x.startswith("page")]
    browser = utils.launch_chromedriver()

    html_pages = []
    for page_name in page_names:
        html_page = utils.extract_page_text_content(browser, (xpdf_path / page_name).resolve())
