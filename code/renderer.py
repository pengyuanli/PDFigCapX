...
def render_pdf(filename, customize_dpi):
    ...
    if os.name == "nt":
        imagemagickPath = (
            "/usr/pengyuan/others/ImageMagick-7.0.3-5-portable-Q16-x86/convert.exe"
        )
        os.system(
            imagemagickPath
            + " -density "
            ...
            + filename
            + " "
            + os.path.join(outputDir, "image.png")
        )
    else:
        os.system(
            ...
            "gs -q -sDEVICE=png16m -o "
            + os.path.join(outputDir, "file-%02d.png")
            + " -r"
            + output_dpi
            + " "
            + filename
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(outputDir, f)) and not f.startswith(".")
    ]
